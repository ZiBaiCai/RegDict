...
function renderSearchResult(result){
  ...
  if(wordsHtml === '')
    wordsHtml = '<p class="no-result">抱歉，没有找到匹配单词</p>';
  else if(result.more)
...
function search(pattern){
  $.get('https://regdict.com/regdict/?key=' + pattern, function(result){
...
function readMore(){
  ...
  $.get('https://regdict.com/regdict/?n=20&key=' + pattern + '&m=' + count, function(result){
    var result = JSON.parse(result);
    var wordsHtml = result.words.map(renderWord).join('');
